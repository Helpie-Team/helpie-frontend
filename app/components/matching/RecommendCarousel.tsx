...
export default function RecommendCarousel({
  ...
  const dummyData = Array(5)
    ...
    .map((_, i) => ({
      ...
      thumbnail: dummyImages[i] as unknown as string,
  ...
  const handleLikeClick = (groupId: number, e: React.MouseEvent) => {
    ...
    const isDummyData =
      forceRender &&
      (!recommendData?.page?.content ||
        recommendData.page.content.length === 0) &&
      groupId >= 0 &&
      groupId <= 4;
            ...
            >
              {displayData.map((meeting) => {
                ...
                return (
                  <div
                    key={meeting.id}
                    className={`w-[231px] h-[220px] flex-shrink-0 rounded-2xl flex flex-col ${
                      !isDummyData && !isLocked
                      ...
                      <div className="relative w-full h-[135px]">
                        ...
                        {isLoggedIn && !isDummyData && (
                          ...
                          >
                            <Image
                              src={
                                likedGroups.has(meeting.id) ? heart : noHeart
                              }
                              alt="찜하기"
                              width={24}
                              height={24}
          ...
          {isLocked && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-gradient-to-t from-white/90 via-white/75 to-white/40 z-20">
              ...
              <div className="flex flex-col gap-1 text-center">
                <h3 className="text-h2 text-black">지금 프로필을 완성해 주세요</h3>
