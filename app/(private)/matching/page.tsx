...
import JoinModal from "@/app/components/matching/modal/JoinModal";
import LoginModal from "@/app/components/matching/modal/LoginModal";

import {
  useGroupListByAuth,
  useToggleGroupMark,
  useSearchPublicGroups,
} from "@/app/hooks/matching/useMatching";
import {
  GroupCategory,
  GroupListItem,
} from "@/app/api/types/matching/matching";

import {
  useMyBookmarkInfo,
  MY_BOOKMARK_INFO_QUERY_KEY,
} from "@/app/hooks/my-page/useMyBookmarkInfo";
...
export default function MatchingCards({
  ...
  const {
    ...
    error: searchError,
  } = useSearchPublicGroups(country, searchKeyword || "", page);

  const {
    data: groupListData,
    isLoading,
    error,
  } = useGroupListByAuth({ country, category, page });

  const isSearchMode = !!searchKeyword && searchKeyword.trim().length > 0;
  const currentData = isSearchMode ? searchData : groupListData;
  const currentLoading = isSearchMode ? isSearchLoading : isLoading;
  const currentError = isSearchMode ? searchError : error;

  // 🔹 로그인한 유저의 관심 목록 가져오기 (하트 초기 상태용)
  const bookmarkQuery = useMyBookmarkInfo({
    size: 100,        // 한 번에 가져올 북마크 개수 (필요시 조절)
    enabled: isLoggedIn,
  });
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64">
        <p className="text-body1-regular text-grayScale-500">
          {isSearchMode ? "검색 중..." : "소모임을 불러오는 중..."}
        </p>
      </div>
    );
  }

  // 에러 상태 - 이미 로드된 데이터가 없을 때만 표시
    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-row flex-wrap gap-4">
        {allMeetings.map((meeting) => {
          ...
          const isLiked = likedGroups.has(meeting.id);
            ...
            >
              ...
              <div className="flex flex-col gap-1 px-2 mt-2">
                <div className="flex items-center gap-2 text-caption1-regular text-grayScale-500">
                  {/* 카테고리 배지 */}
                  <div
                    className={`${categoryColor} text-white px-2 py-1 rounded-full text-caption1-b whitespace-nowrap flex-shrink-0`}
                  >
                    {categoryDisplay}
                  </div>
                ...
                <h3 className="text-body2 text-black line-clamp-1 break-words">
                  {meeting.title}
                </h3>
